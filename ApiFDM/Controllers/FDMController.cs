...
namespace ApiFDM.Controllers
{
    ...
    public class FDMController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/fdm/route/{ymf}/{ymt}")]
        public async Task<DataResponse> GetFDMRoute(int ymf, int ymt)
        {
        ...
        [HttpGet]
        [Route("api/fdm/dashboard/eventname/{ymf}/{ymt}")]
        public async Task<DataResponse> GetAllFDMEventName(int ymf, int ymt)
        {
            var query = from x in context.FDMCptEventMonthlies
        ...
        [HttpGet]
        [Route("api/fdm/dashboard/all/events/{ymf}/{ymt}")]
        public async Task<DataResponse> GetEventsDaily(int ymf, int ymt)
        {
        ...
        public async Task<DataResponse> GetRegMonthly(int ymf, int ymt, string reg)
        {
            var query = from x in context.FDMRegMonthlies
                        ...
                        select new
                        {
                            ...
                            EventPerFlight = grp.Sum(q => q.EventCount) * 100.0 / grp.Sum(q => q.HighCount) * 4 + grp.Sum(q => q.MediumCount) * 2 + grp.Sum(q => q.LowCount) * 1.0,
                            ScorePerEvent = grp.Sum(q => q.ScorePerEvent)
                        };
        ...
        [HttpGet]
        [Route("api/fdm/register/monthly/{ymf}/{ymt}")]
        public async Task<DataResponse> GetEventByRegMonthly(int ymf, int ymt)
        {
            try
            {
                var query = from x in context.FDMRegMonthlies
        ...
        public async Task<DataResponse> GetTopCpt(int ymf, int ymt)
        {
        ...
        public async Task<DataResponse> CompareTopCpt(int ymf, int ymt)
        {
            ...
            try
            {
                var qry_max = (from x in context.FDMCptMonthlies
                               where x.YearMonth >= ymf && x.YearMonth <= ymt
                               group x by new { x.CptCode, x.CptId, x.CptName } into grp
                               select new
                               {
                                   grp.Key.CptCode,
                                   grp.Key.CptId,
                                   grp.Key.CptName,
                                   TotlaScore = grp.Sum(q => q.ScorePerFlight),
